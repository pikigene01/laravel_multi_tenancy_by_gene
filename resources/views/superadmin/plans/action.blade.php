@if ($plan->active_status != 1)
    <a class="btn btn-sm small btn btn-success " href="plan-status/{{ $plan->id }}" data-bs-toggle="tooltip"
        data-bs-placement="bottom" title="" data-bs-original-title="{{ __('Active') }}"
        aria-label="{{ __('Active') }}"><i class="ti ti-checks text-white"></i></a>
@else
    <a class="btn btn-sm small btn btn-danger " href="plan-status/{{ $plan->id }}" data-bs-toggle="tooltip"
        data-bs-placement="bottom" title="" data-bs-original-title="{{ __('Deactive') }}"
        aria-label="{{ __('Deactive') }}"><i class="ti ti-ban text-white"></i></a>
@endif
@can('edit-plan')
    <a class="btn btn-sm small btn btn-warning " href="plans/{{ $plan->id }}/edit" data-bs-toggle="tooltip"
