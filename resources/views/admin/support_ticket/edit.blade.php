    ...
    <div class="float-end">
        <div class="d-flex align-items-center">
            <button type="button" class="btn btn-sm btn-primary" id="ticket-info" data-bs-toggle="tooltip"
                ...
                aria-label="{{ __('Edit') }}"><i class="ti ti-edit text-white"></i></button>
        </div>
    </div>
@endsection
@section('content')
    <div class="row ticket-info d-none">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            {{ Form::model($supportticket, [
                'route' => ['support-ticket.update', $supportticket->id],
                'method' => 'PUT',
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <small>
                            <span class="text-right">
                                {{ __('Status') }} : <span
                                    class="badge rounded-pill @if ($supportticket->status == 'In Progress') badge bg-warning  @elseif($supportticket->status == 'On Hold') badge bg-danger @else badge bg-success @endif">{{ $supportticket->status }}</span>
                            </span>
                        </small>
                    </div>
                </div>
                                ...
                                @foreach ($attachments as $index => $attachment)
                                    <li class="list-group-item px-0">
                                        {{ $attachment }} <a
                                            ...
                                            class="edit-icon py-1 ml-2" download><i class="fas fa-download ms-2"></i></a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                                    ...
                                    @foreach ($attachments as $index => $attachment)
                                        <li class="list-group-item px-0">
                                            {{ $attachment }}<a download=""
                                                href="{{ asset(Storage::url('tickets/' . $supportticket->ticket_id . '/' . $attachment)) }}"
                                                class="edit-icon py-1 ml-2" data-bs-toggle="tooltip"
                                                data-bs-placement="bottom" target="_new" title=""
                                                data-bs-original-title="{{ __('Download') }}"
                                                aria-label="{{ __('Download') }}"><i class="fa fa-download ms-2"></i></a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
